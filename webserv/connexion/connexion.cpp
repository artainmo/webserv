...
void setup_server(t_server &s, t_config &config)
{
	...
	// if(setsockopt(s.server_socket, SOL_SOCKET, SO_NOSIGPIPE, (char *)&opt, sizeof(opt)) == -1)
	// {
	// 	std::cout << "Error: sotsockopt failed" << std::endl;
	// 	exit(1);
	// }
...
std::string get_client_request(t_server &s, t_config &config)
{
	...
	for (unsigned int i = 0; i < SOMAXCONN; i++)
	{
		...
		{
			if (send(s.client_socket[i], s.reply[s.client_socket[i]].c_str(), s.reply[s.client_socket[i]].size(), 0) == -1)
			{
				std::cout << "Error: send failed" << std::endl;
				//exit(1);
			}
			s.reply.erase(s.client_socket[i]);
		}
		...
		{
			message_len = recv(s.client_socket[i] , message_buffer, 1024, MSG_PEEK); //Read the incoming message
...
void new_incoming_connection(t_server &s, t_config &config)
{
	...
	fcntl(s.connected_socket, F_SETFL, O_NONBLOCK); //fcntl to make the fd NON-blocking
...
void wait_connexion(t_server &s, t_config &config)
{
	...
	if (select(FD_SETSIZE , &s.active_socket_read , &s.active_socket_write , NULL , NULL) == -1)// && (errno!=EINTR))
	{
